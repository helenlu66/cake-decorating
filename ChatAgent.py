...
class ChatAgent:
        ...
        if parsed_message[0].lower() == 'action':
            ...
            # action status is success, ask the human what they like to do next
                ...
                if not self.response_enabled: # robot not allowed to talk. Return a system status message
                    return "The robot has successfully completed the action"
            ...
            # action status is failure, ask to try another action
                ...
                if not self.response_enabled: # robot not allowed to talk. Return a system status message
                    return "The robot has failed to complete the action"
    ...
    def generate_suggestion(self, following_an_action=True):
        ...
        if following_an_action:
            self.messages.append(SystemMessage(content=
                    """You just completed an action. Suggest an action you can take next in the following format:
                    ```
                    suggestion
    ...
    def generate_random_suggestion(self, following_an_action=True):
        ...
        suggestion = self.random_suggestion_rephraser.run({'sentences':suggestion})
